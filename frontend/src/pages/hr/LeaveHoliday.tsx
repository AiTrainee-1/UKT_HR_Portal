...
export default function LeaveHoliday() {
  ...
  const filteredLeaves = (leaves ?? []).filter(
    (l) => !l.isHalfDay && (filterStatus === "all" || l.status === filterStatus),
  );
  const halfDayLeaves = (leaves ?? []).filter((l) => l.isHalfDay);
  const filteredHalfDayLeaves = halfDayLeaves.filter(
    (l) => halfDayFilterStatus === "all" || l.status === halfDayFilterStatus,
  );
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {(activeTab === "halfDay"
            ? [
                {
                  label: "Pending",
                  value: filteredHalfDayLeaves.filter((l) => l.status === "pending").length,
                  color: "text-amber-700 bg-amber-50 border-amber-100",
                },
                {
                  label: "Approved",
                  value: halfDayLeaves.filter((l) => l.status === "approved").length,
                  color: "text-green-700 bg-green-50 border-green-100",
                },
                {
                  label: "Rejected",
                  value: halfDayLeaves.filter((l) => l.status === "rejected").length,
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <div className="flex gap-0.5">
                                  {Array.from({ length: p.monthlyLimit }).map((_, i) => (
                                    <div
                                      key={i}
                                      className={`w-4 h-1.5 rounded-full ${i < p.monthlyUsed! ? "bg-amber-400" : "bg-gray-200"}`}
                                    />
                                  ))}
                                </div>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4 py-2">
                  ...
                  <div className="flex gap-3 pt-2">
                    <Button variant="outline" className="flex-1" onClick={() => setShowPermDialog(false)}>
                      Cancel
                    </Button>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-gray-900">
                          {selectedLeave.employeeCode ?? `#${selectedLeave.employeeId}`}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className="space-y-3">
                  ...
                  {selectedLeave.hrComment && (
                    <div>
                      ...
                      <p className="text-sm text-blue-700 bg-blue-50 rounded-lg p-3 border border-blue-100">
                        {selectedLeave.hrComment}
                      </p>
                    </div>
                  )}
                  {selectedLeave.approvedBy && (
                    <p className="text-xs text-gray-500">
                      ...
                      <strong>{selectedLeave.approvedBy}</strong>
                      {selectedLeave.approverRole === "dept_head" ? " (Department Head)" : " (HR)"}
                    </p>
                  )}
                  <p className="text-xs text-gray-300">
                    Submitted:{" "}
                    {selectedLeave.createdAt ? new Date(selectedLeave.createdAt).toLocaleString("en-IN") : "—"}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs text-gray-400">Employee ID</p>
                        <p className="text-sm font-semibold text-gray-900">
                          {selectedPerm.employeeCode ?? `#${selectedPerm.employeeId}`}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {selectedPerm.date
                          ? new Date(selectedPerm.date).toLocaleDateString("en-IN", {
                              weekday: "short",
                              day: "numeric",
                              month: "long",
                              year: "numeric",
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {selectedPerm.permissionTime ?? "Not specified"}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Status</p>
                  ...
                  {selectedPerm.approvedBy && (
                    <p className="text-xs text-gray-500">
                      {selectedPerm.status === "rejected" ? "Rejected By" : "Approved By"}:{" "}
                      <strong>{selectedPerm.approvedBy}</strong>
