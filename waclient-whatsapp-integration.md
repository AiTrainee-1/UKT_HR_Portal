...
| Punch reminder | A friendly heads-up **5 minutes before** each expected punch (HR can change the 5), only if that punch is still missing: check-in at the shift start, lunch-out at the shift's first-half end, lunch-in one lunch duration later, check-out at the shift end. This is the same in simple and strict attendance mode (simple mode only ignores lunch punches when working out the day's pay); a shift with no lunch break has just check-in and check-out. Greeting by time of day, wording per punch, and a short motivational line. |
